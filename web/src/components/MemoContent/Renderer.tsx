import CustomHiddenBlock from "@/forked/components/MemoContent/CustomHiddenBlock";
import CustomHiddenInline from "@/forked/components/MemoContent/CustomHiddenInline";
import { resolveNode } from "@/forked/resolve";
import {
  AutoLinkNode,
  BlockquoteNode,
  BoldItalicNode,
  ...
  NodeType,
  Node,
  CustomHiddenInlineNode,
  CustomHiddenBlockNode,
...
const Renderer: React.FC<Props> = ({ index, node }: Props) => {
  const resolvedNode = resolveNode(node) || ({} as Node);

  if (Array.isArray(resolvedNode)) {
    return resolvedNode.map((_node, i) => _node && <Renderer key={`${index}-${i}`} index={`${index}-${i}`} node={_node} />);
  } else {
    node = resolvedNode;
  }
  ...
  switch (node.type) {
    ...
    case NodeType.CUSTOM_HIDDEN_INLINE:
      return <CustomHiddenInline index={index} {...(node.customHiddenInlineNode as CustomHiddenInlineNode)} />;
    case NodeType.CUSTOM_HIDDEN_BLOCK:
      return <CustomHiddenBlock index={index} {...(node.customHiddenBlockNode as CustomHiddenBlockNode)} />;
    default:
      return null;
  }
};
